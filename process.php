    ...
    function show(){
     ...
     if($result->num_rows>0){
        ...
        while($data=$result->fetch_assoc()){
            ...
            <td>'.$data['email'].'</td>
            <td>'.$data['status'].'</td>
            <td><button class"btn btn-info"  id="updateid" value="'.$data['id'].'">Update</button> </td>
             <td><button class"btn btn-info" data-bs-toggle="modal" data-bs-target="#deletemodal" id="deleteeid" value="'.$data['id'].'">Delete</button> </td>
            </tr>';
        }
        $output.='</table>';
        echo $output;
